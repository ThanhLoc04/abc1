...
module.exports.checkPath = function (type, senderID) {
    ...
    const pathEquipment = require("./cauca/equipment.json");
...
module.exports.handleReply = async function ({
    ...
    case 'shop': {
        ...
        if (body == 2) {
            ...
            return api.sendMessage(`Chọn loại cá muốn bán:\n1. Common - ${Common.length}\n2. Uncommon - ${Uncommon.length}\n3. Rare - ${Rare.length}\n4. Epic - ${Epic.length}\n5. Legendary - ${Legendary.length}\n6. Mythical - ${Mythical.length}\n7. Spectral - ${Spectral.length}\n8. Etherial - ${Etherial.length}\n9. Unknown - ${Unknown.length}\n10. Unknown - ${Unreal.length}`, threadID, (error, info) => {
                global.client.handleReply.push({
                    name: this.config.name,
                    messageID: info.messageID,
                    author: event.senderID,
        ...
        if (body == 4) {
            ...
            return api.sendMessage(listItem + 'Reply tin nhắn này để chọn phụ kiện muốn mua', threadID, (error, info) => {
                global.client.handleReply.push({
                    name: this.config.name,
                    messageID: info.messageID,////// all
                    author: event.senderID,
    ...
    case 'equipMain': {
        ...
        if (parseInt(body) > equip.length || parseInt(body) <= 0) return api.sendMessage('Lựa chọn không hợp lệ!', threadID, messageID);
        api.unsendMessage(handleReply.messageID)
        data.accessory = equip[parseInt(body) - 1].name
        writeFileSync(this.checkPath(3, senderID), JSON.stringify(data, null, 2));
        return api.sendMessage(`===PHỤ KIỆN===\n Đã trang bị '${equip[parseInt(body) - 1].name}'!`, threadID, messageID);
    }
    case 'location': {
        const data = require("./cauca/data.json");
