...
module.exports = function ({ api, models, Users, Threads, Currencies }) {
   ...
   const path = require('path')
     ...
     const prefixRegex = new RegExp(`^(<@!?${senderID}>|${escapeRegex((threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : PREFIX )})\\s*`);
         ...
         if(dataAdbox.adminbox.hasOwnProperty(threadID) && dataAdbox.adminbox[threadID] == true && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true ) return api.sendMessage('[ MODE ] - Chỉ Admin với Qtv nhóm mới được sử dụng bot!!', event.threadID, event.messageID)
        ...
        const config = require('./../../config.json');
           const girl = require('./../../includes/video.json');
           var image = girl[Math.floor(Math.random() * girl.length)].trim();
           var stream = (await axios.get(image, { responseType: 'stream' })).data;
